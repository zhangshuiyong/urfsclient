...
pub fn prepare_cmd_args(bti_string: &'static str) -> App {
...
pub fn init_log(matches: &ArgMatches) -> Result<()> {
...
lazy_static! {
   pub static ref BTI_STRING: String = get_build_time_info().0;
   pub static ref BTI: BuildTimeInfo = get_build_time_info().1;
}

fn get_build_time_info() -> (String, BuildTimeInfo) {
...
//     let arg_vec = vec!["nydus-image","check","-B", "/Users/terrill/Documents/urchin/urchin2/urfs/tests/texture/bootstrap/rafs-v5.boot","-v"];
...
impl Command {
    pub fn create(matches: &ArgMatches, build_info: &BuildTimeInfo) -> Result<()> {
    ...
    pub fn merge(matches: &ArgMatches, build_info: &BuildTimeInfo) -> Result<()> {
    ...
    pub fn compact(matches: &ArgMatches, build_info: &BuildTimeInfo) -> Result<()> {
        let config =
            Self::get_configuration(matches).context("failed to get configuration information")?;
        config
            .internal
    ...
    pub fn unpack(matches: &ArgMatches) -> Result<()> {
    ...
    pub fn check(matches: &ArgMatches, build_info: &BuildTimeInfo) -> Result<()> {
    ...
    pub fn inspect(matches: &ArgMatches) -> Result<()> {
        let bootstrap_path = Self::get_bootstrap(matches)?;
        let mut config = Self::get_configuration(matches)?;
        // For backward compatibility with v2.1
        config
    ...
    pub fn stat(matches: &ArgMatches) -> Result<()> {
